...
class AuthService:
    ...
    async def login(
        ...
        # Locked account check.  SQLite (used by the local test/development
        # setup) may return a naive datetime even for a timezone-aware column.
        # Treat legacy naive values as UTC so a lockout never turns into a 500.
        lockout_until = officer.lockout_until
        if lockout_until and lockout_until.tzinfo is None:
            lockout_until = lockout_until.replace(tzinfo=datetime.timezone.utc)

        if lockout_until and lockout_until > datetime.datetime.now(datetime.timezone.utc):
            await self._log_login(officer.id, ip_address, user_agent, success=False)
